...
void add_ticket(Ticket t, vector<Ticket> &a, int &k) {
    a.push_back(t);
    ++k;
}

int train_num(int t, string d, vector<Ticket> &a, int k) {
    int n = 0;
    for (int i = 0; i < k; ++i) {
...
void delete_info(int n, vector<Ticket> &a, int L) {
    a[n] = Ticket();
    for (int i = n; i < L - 1; ++i) {
        a[i] = a[i + 1];
    }
...
int van_num(int t, int v, vector<Ticket> &a, int L) {
    int n = 0;
    for (int i = 0; i < L; ++i) {
